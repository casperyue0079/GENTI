...
电脑小白可以直接参考运行指南：https://github.com/casperyue0079/GENTI/blob/main/%E8%BF%90%E8%A1%8C%E6%8C%87%E5%8D%97.%20md 
...
1. 总体架构
单一事实源：运行时与保存文件都用 Pydantic 模型 TestProject（core/models.py）：配置、两种模式各自的维度/轴、题目、普通结果、稀有项、图片路径等。
应用层：Streamlit 多步向导（app.py + steps/step*.py）读写 session_state.project，负责编辑与调用 AI。
生成层：通过兼容 OpenAI 的 API 产出/补全结构化内容，再校验进 TestProject（具体 Prompt 在 core/ 的生成逻辑里，与判分分离）。
判分层：多向轴 → core/scoring.py；多维度 → core/dim_scoring.py。导出静态站时把项目压平成 JSON payload，由前端 engine.js 复现同一套公式（core/exporter.py 的 _flatten_project）。
2. 多向轴模式（MULTI_AXIS，算法参考mbti）
...
normalized[d] = clip(raw[d] / max_abs[d], -1, 1)
